      ...
      <div className={`bg-primary text-primary-foreground/80 text-[0.65rem] uppercase tracking-[0.15em] py-2 px-6 flex justify-between items-center transition-all duration-500 ${scrolled ? 'h-0 opacity-0 overflow-hidden py-0' : 'h-auto opacity-100'}`}>
        ...
        <div className="flex gap-6 items-center w-full md:w-auto justify-end font-semibold">
          ...
          <Button 
            ...
            className="h-6 text-xs font-bold px-3 ml-2 rounded-lg cursor-pointer"
          >
      ...
      <nav className="py-3 px-6 md:px-12 w-full flex justify-between items-center border-b border-primary/5">
        ...
        <div className="flex items-center gap-8">
          <div className="hidden lg:flex items-center gap-8 text-xs font-bold uppercase tracking-[0.1em] text-primary/80">
            ...
            <Link href="/academics" className="hover:text-primary transition-colors">{t("academics")}</Link>
            <Link href="/admissions" className="hover:text-primary transition-colors">{t("admissions")}</Link>
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="right" className="bg-white/95 backdrop-blur-2xl border-l border-primary/10 p-10 flex flex-col sm:max-w-md shadow-2xl">
              ...
              <div className="flex flex-col gap-5 text-sm font-bold tracking-[0.15em] uppercase text-primary/80">
                <Link href="/" onClick={() => setIsOpen(false)} className="hover:text-secondary hover:translate-x-2 transition-all duration-300">{t("home")}</Link>
                <Link href="/about" onClick={() => setIsOpen(false)} className="hover:text-secondary hover:translate-x-2 transition-all duration-300">{t("about")}</Link>
                <Link href="/academics" onClick={() => setIsOpen(false)} className="hover:text-secondary hover:translate-x-2 transition-all duration-300">{t("academics")}</Link>
                <Link href="/admissions" onClick={() => setIsOpen(false)} className="hover:text-secondary hover:translate-x-2 transition-all duration-300">{t("admissions")}</Link>
                <Link href="/contact" onClick={() => setIsOpen(false)} className="hover:text-secondary hover:translate-x-2 transition-all duration-300">{t("contact")}</Link>
              </div>

              <div className="mt-auto pt-8 border-t border-primary/10 flex flex-col gap-4">
                <Button asChild className="rounded-lg bg-secondary text-primary hover:bg-primary hover:text-white font-bold tracking-widest uppercase text-xs w-full transition-all duration-300">
                  <Link href="/portal" onClick={() => setIsOpen(false)}>{t("apply")}</Link>
                </Button>
              </div>
            </SheetContent>
          </Sheet>
